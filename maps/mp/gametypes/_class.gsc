...
get_specialtydata( class_num, specialty )
{
	...
	if( specialty == "specialty1" )
	{
		if( isSubstr( cac_group, "grenade" ) )
		{
			...
			{
				self.custom_class[class_num]["specialgrenades"] = self.custom_class[class_num]["special_grenade"];
				self.custom_class[class_num]["specialgrenades_count"] = 2;
			}
			return;
		}
		else
...
replenishLoadout() // used by ammo hardpoint.
{
	...
	// For some reason hasPerk isn't returning true for the x3 nade perks..
	...
	for( i=0; i < perks.size; i++ )
	{
		...
		if ( perk == "specialty_specialgrenade" )
		{
			extraSpecials = true;
		}

		if ( perk == "specialty_fraggrenade" )
		{
			extraGrenades = true;
		}
	}

	//self iPrintlnbold( "^3Bandolier: ^7" + extraAmmo + " ^1Special x3: ^7" + extraSpecials + " ^2Frag x3: ^7" + extraGrenades );

	weaponsList = self GetWeaponsList();
	for( idx = 0; idx < weaponsList.size; idx++ )
	{
		weapon = weaponsList[idx];
		if ( weapon == "claymore_mp" || weapon == "claymore_detonator_mp" || weapon == "c4_mp" )
		{
			if( extraAmmo || self hasPerk( "specialty_extraammo" ) )
			{
				self setWeaponAmmoStock( weapon, 6 );
			}
			else
			{
		...
		{
			if( extraGrenades || self hasPerk( "specialty_fraggrenade" ) )
			{
		...
		else if ( weapon == "concussion_grenade_mp" || weapon == "flash_grenade_mp" || weapon == "smoke_grenade_mp" )
		{
			if( extraSpecials && weapon != "smoke_grenade_mp" || self hasPerk( "specialty_specialgrenade" ) )
			{
				self SetWeaponAmmoClip( weapon, 6 );
			}
			else
