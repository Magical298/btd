...
//================================================================================================
// Zombotron: Gas grenade
	level.gasPoisonRadius = 250;
	level.gasDuration  = 12;
	level.poisonDuration = 14;
//================================================================================================
...
//================================================================================================
// Zombotron: Infinite ammo thread
...
isHackWeapon( weapon )
{
	if ( weapon == "radar_mp" || weapon == "airstrike_mp" || weapon == "helicopter_mp" || weapon == "blackhawk_mp" || weapon == "ac130_mp" || weapon == "napalm_mp" || weapon == "nuke_mp" )
	{ return true; }
...
//================================================================================================
// Zombotron: Don't drop specials
	if( weapon == "special1_mp" || weapon == "special2_mp" || weapon == "special3_mp" || weapon == "special4_mp" || weapon == "special5_mp" || weapon == "special6_mp" )
	{ return true; }

	// Don't drop misc vehicle weapons etc.
	if( weapon == "gl_mp" || weapon == "defaultweapon_mp" || weapon == "briefcase_bomb_defuse_mp" || weapon == "cobra_FFAR_mp" || weapon == "hind_FFAR_mp" )
	{ return true; }
//================================================================================================
	return false;
}
...
//================================================================================================
// Zombotron: Added for special grenades
	else if (weaponName == "smoke_grenade_mp" )
	{
		grenade thread watchGasGrenadeDetonation( self );
		grenade.originalOwner = self;
		//self iPrintLn("^1Tracking ^2Gas Grenade!");
	}
...
watchGasGrenadeDetonation( owner )
{	
	self waittill( "explode", position );

	level.gasPoisonRadius = 200;
	level.gasDuration = 12;
	level.poisonDuration = 14;

	gasEffectArea = spawn("trigger_radius", position, 0, level.gasPoisonRadius, level.gasPoisonRadius*2);

	loopWaitTime = 0.5;
	durationOfGas = level.gasDuration;

	//owner iPrintLn("^2Gas Grenade ^1Detonated!");

	while( durationOfGas > 0 )
	{
		...
		for (i = 0; i < zombies.size; i++)
		{
			...
			if( isDefined( zom ) )
			{
				if ( !isDefined(zom.poisoned) || zom.poisoned == false )
				{
					if ( zom istouching(gasEffectArea) )
					{
						//owner iPrintLn("^2Gasing Zombie!");
						zom thread startPoisoning( gasEffectArea, owner );
					}
				}
			}
		}
		...
		wait( loopWaitTime );
		durationOfGas -= loopWaitTime;
	}

	if ( level.gasDuration < level.poisonDuration )
	{
		wait( level.poisonDuration - level.gasDuration );
	}

	//owner iPrintLn("^2Gas Grenade ^1empty");
	gasEffectArea delete();	
}

startPoisoning(gasEffectArea, source)
{
	self endon( "death" );
	self endon( "game_ended" );

	if( self.isNormalzom == true )
	{
		self notify("damage", int(self.health * 0.15), source, (0,1,0), self.origin, "MOD_GRENADE", "", "", "", 1);
		return;
	}

	self.poisoned = true;

	while( isdefined(gasEffectArea) )
	{
		//source iPrintLn("^2Zombie taking ^1poison damage!");
		self notify("damage", int(self.maxhealth * 0.15), source, (0,1,0), self.origin, "MOD_GRENADE", "", "", "", 1);
		wait(1);
	}
...
{
	self endon("death");
	self.distance = level.claymoreDetonateRadius + 1;
	...
	while(1)
	{
		...
		for(z = 0; z < level.zombies.size; z++)
		{
			if( !isDefined( level.zombies[z] ) )
			{ continue; }

			self.distance = distance(self.origin, level.zombies[z].origin);

			if ( self.distance < level.claymoreDetonateRadius )
				break;
		}
...
getDamageableEnts(pos, radius, doLOS, startRadius)
{
	...
	for (i = 0; i < zombies.size; i++)
	{
		...
		zombiepos = zombies[i].origin + (0,0,32);
		dist = distance(pos, zombiepos);
		if (dist < radius && (!doLOS || weaponDamageTracePassed(pos, zombiepos, startRadius, undefined)))
		{
			newent = spawnstruct();
			newent.isPlayer = false;
			newent.isADestructable = false;
			newent.entity = zombies[i];
			newent.damageCenter = zombiepos;
...
damageEnt(eInflictor, eAttacker, iDamage, sMeansOfDeath, sWeapon, damagepos, damagedir)
{
	...
	{
		self.entity notify("damage", iDamage, eAttacker, damagedir, damagepos, sMeansOfDeath, "", "" );
	}
	else
	{
