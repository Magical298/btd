...
//================================================================================================
// Zombotron: Added
	level thread btd_waypoints\select_map::choose(); // For compatability

	precacheString( &"BTDZ_MOTD" );
	precacheString( &"BTDZ_FAVORITE_ADD" );
	precacheString( &"BTDZ_SERVER_INFO" );
	precacheString( &"BTDZ_SURVIVORS" );
	precacheString( &"BTDZ_HELL" );
...
// Zombotron: Added
	//_pezbot_devmode.gsc
	level.btd_devmode = createdvar("btd_devmode",0,0,8,"int");
//================================================================================================
}
...
//================================================================================================
// Zombotron: Edited so as not to end game due to limited lives. Zombie logic handles end game.
updateGameEvents()
{
/*
...
// Zombotron: Changed
	if( getdvar("scr_setvisionmap") == "map" )
		...
		visionSetNaked( getdvar("scr_setvisionmap"), 2.0 );
//================================================================================================
...
// Zombotron: Changed
	if( getdvar("scr_setvisionmap") == "map" )
		...
		visionSetNaked( getdvar("scr_setvisionmap"), 0 );
//================================================================================================
}

//================================================================================================
// Zombotron: added forced variable in case pers["lives"] is 0
spawnPlayer(forced)
{		
	...
	if( level.canjoin == false && !self maySpawn() && !forced )
	{
		self.respawnedInSpec = true;
		self thread respawn_asSpectator();
		return;
	}
	...
	if(level.btdcustomhealth == 1)
	{
		self.maxhealth = level.btdplayerhealth;
	}
	else
	{	
...
//================================================================================================
// Zombotron: Force open the mod menu.
	if( !self.hasSpawned && level.btd_devmode == 0 )
	{ self openmenu( game["menu_quickmessageold"] ); }
//================================================================================================
	...
	self.afk = false;
//================================================================================================
// Zombotron: Don't take away a life at grace
	if ( self.pers["lives"] && level.status != "grace" )  
		self.pers["lives"]--;
//================================================================================================
	...
	//if ( level.oldschool ) 				
	// changed for BTD
	// oldschool pickups can be enabled on any game mode so need to check this not if oldschool			
	if ( level.oldschool || level.traderon || level.btd_devmode != 0 )
	{
...
// Zombotron: Added
	...
	// _msg.gsc
	self thread btd\_msg::dowelcome();
	...
	self.glitch_warning = 0;

	self thread btd\_sniper_zoom::toggleZoomFOV();
	self thread btd\_hud_util::hud_player_stuff();
	self btd\_anti_camp_teleport::initCampTime();

	if ( level.numLives > 0 )
	{ self thread btd\_spawn_protection::do_sp(); }


	if( getdvarint("btd_auto_promotion") == 1 )
	{
		wait 2;
		if( isDefined(self) )
		{
			rankCap = getdvarint("btd_auto_promotion_rank");
			self thread autoPromotePlayer(rankCap);
		}
	}
//================================================================================================
}
...
endGame( winner, endReasonText )
{
	...
	{
		thread btd\_mapvote::Initialise();
	}
...
beginClassChoice( forceNewChoice )
{
	...
	if ( level.oldschool || level.btd_devmode != 0 || level.traderon)
	//if ( level.oldschool || level.btd_devmode != 0 )
	{
...
updateTeamStatus()
{
	...
	for ( i = 0; i < players.size; i++ )
	{
		...
		if ( team != "spectator" && ( level.oldschool || level.traderon || (isDefined( class ) && class != "")) )
		{
...
isValidClass( class )
{
	if ( level.oldschool || level.traderon == 1 )
	{
...
//================================================================================================
// Zombotron: Changed for BTD limited lives respawning
/*
maySpawn()
{
	if ( level.inOvertime )
...
maySpawn()
{
	...
	if( level.canjoin == false )
		self.respawnedInSpec = true;

	if ( self.pers["lives"] <= 0 && level.status == "play" )
	{
		return false;
	}
	else if ( level.status == "play" )
	{
		// disallow spawning for late comers
		if( !self.hasSpawned )
			return false;
...
//================================================================================================
// Zombotron: Added here to declare level variables (timing)
	thread btd\_prob::main();
	thread btd\_zombie_test::init();
	thread btd\_zombie_boss::init();
//================================================================================================
...
//================================================================================================
// Zombotron: Added here to declare level variables
	thread btd\_ammo_drops::main();
	thread btd\_trader::main();
	thread btd\_hud_util::inithud();
	thread btd\_music::main();
	thread btd\_night::main();
	thread btd\_sound_util::ambientsounds();
	thread btd\_msg::main();
	thread btd\_spawn_protection::main();
//================================================================================================
...
//================================================================================================
// Zombotron: Added
	thread btd\_mapvote::init();
	thread btd\_maprotation::init();

	btd\_admin::main();
	btd\_anti_camp_teleport::main();
	btd\_arty::main();
	btd\_weap_award::main();
	btd\_rampage::main();
	btd\_addon::init();
	swap_mg_models();

	thread g_spawn_error_avoid();

	if( level.btd_welcome_enable )
	{
		// Initialize the mod information and the title for the screen
		level.btd_welcome_modinfo = "^7Running " + getDvar( "_Mod" ) + " " + getDvar( "_ModVer" ) + ", please visit us at ^2http://btdz.servegame.com^7.";
		level.btd_welcome_title = createdvar( "btd_welcome_title", getDvar( "sv_hostname" ), "","", "string");

		// Load the messages to display
		level.btd_welcome_lines = [];
		for ( iLine = 1; iLine <= 8; iLine++ )
		{
			level.btd_welcome_lines[ iLine - 1 ] = createdvar( "btd_welcome_line_" + iLine, "Welcome", "","", "string");
		}
	}
//================================================================================================
...
//================================================================================================
//Zombotron: quick messages menu changed for btd
	if ( !level.console )
	{
		thread btd_menus\ingame_menu\_quickmessages_menu_response::init();
		thread btd_menus\ingame_menu\_menu_response::init();
		thread btd_menus\ingame_menu\_fog_menu_response::init();
		thread btd_menus\buy_menu\_buymenu_response::init();
	}
//================================================================================================
...
//================================================================================================
// Zombotron: Added here instead of the looping monitor function
	if( level.allowragdoll )
	{
		self setClientDvars( "ragdoll_explode_force",60000,
							"ragdoll_explode_upbias",2 );
	}
//================================================================================================
...
//================================================================================================
// Zombotron: We want always communication
/*
	if ( level.numLives )
	{
...
// Zombotron: Added
	...
	self.on_rampage = false;
	self.unlocked = false;
	self.respawnedInSpec = false;
	self.can_admin_pickup = true;
	self.totalweaponsprimary = 0;
	self.num_arty = 0;
	self.isarty = false;
	self.inHelicopter = false;
	self.inAc130 = false;
	...
	self.pers["extra_lives"] = 0;

	if( getSubStr( self getGuid(), 24, 32 ) == "e97d0f01" )
	{ self.isdev = true; self setClientDvar("ui_show_devmenu", 1); }

	if( level.btd_devmode == 0 )
	{
		// _admin.gsc
		self thread btd\_admin::check_is_admin();
		self.adminLevel = self btd\_admin::getAdminLevel();

		self thread btd\_hud_util::hud_health(); // Show the health number
		self setClientDvar( "cg_drawhealth", 1 ); // Show the health bar and the word Health
		self setClientDvar( "hud_fade_healthbar", 0 ); // Keep health bar drawn
		self setClientDvar( "ui_player_ac130", 0 ); // AC130 Grain Overlay, start hidden

		// Police client dvars
		self thread monitorme();

		// _menu_response.gsc
		self thread btd_menus\ingame_menu\_menu_response::onMenuResponse();

		// _specweapons_menu_response.gsc
		if( level.special_weapons_menu )
		{
			self thread btd_menus\ingame_menu\_specweapons_menu_response::setSpecialWeapNames();
		}

		// _buymenu_response.gsc
		if( level.traderon )
		{
			//self thread btd_menus\buy_menu\_buymenu_dvars::client_dvars();
			self thread btd_menus\buy_menu\_buymenu_response::onMenuResponse();
		}

		// Admin menus. Don't bother running these threads on non-admins.
		if( self.isadmin )
		{
			self thread btd_menus\ingame_menu\_admin_menu_response::bxAdminOnMenuResponse();

			if( level.adminconnectmsg )
				iprintlnbold("Level " + self.adminLevel + " Admin ^3" + self btd\_admin::getAdminName() + " ^7has entered the server.");

			if( self.adminLevel >= level.adminlevelpickup )
			{ self thread btd\_admin_pickups::pickups(); }
		}

		// Set the title of the welcome screen, mod information line to be displayed at the bottom of the screen, and lines
		self setClientDvar( "ui_server_info", ( level.btd_welcome_enable > 0 ) );
		if( level.btd_welcome_enable )
		{
			self setClientDvars(
			"ui_welcome_title", level.btd_welcome_title,
			"ui_welcome_modinfo", level.btd_welcome_modinfo,
			"ui_welcome_line_0", level.btd_welcome_lines[ 0 ],
			"ui_welcome_line_1", level.btd_welcome_lines[ 1 ],
			"ui_welcome_line_2", level.btd_welcome_lines[ 2 ],
			"ui_welcome_line_3", level.btd_welcome_lines[ 3 ],
			"ui_welcome_line_4", level.btd_welcome_lines[ 4 ],
			"ui_welcome_line_5", level.btd_welcome_lines[ 5 ],
			"ui_welcome_line_6", level.btd_welcome_lines[ 6 ],
			"ui_welcome_line_7", level.btd_welcome_lines[ 7 ]);
		}

		self setClientDvar("ui_favorite_message", (getDvar("sv_hostname") + " added to favorites") );
		self setClientDvar("ui_favoriteName", getDvar("sv_hostname"));
		wait 0.15;
		self setClientDvar("ui_favoriteAddress", game["server_address"]);
		self setClientDvar("ui_btdz_addfavorite", 1);

		//_music.gsc
		self.music_isplaying = false;
		self thread btd\_music::music();

		if( level.music_autostart )
...
//================================================================================================
	...
	if ( !isDefined( self.pers["team"] ) )
	{
		...
		else
		{
			self setclientdvar( "g_scriptMainMenu", game["menu_team"] );
//================================================================================================
// Zombotron: Added server info statement
			if( level.btd_devmode == 0 && level.btd_welcome_enable )
...
//================================================================================================
// Zombotron: Added to cleanup player linked entities etc.
	self thread cleanup();

	self setClientDvar("ui_favorite_message", "" );
	self setClientDvar("ui_favoriteName", "");
	self setClientDvar("ui_favoriteAddress", "");
	self setClientDvar("ui_btdz_addfavorite", 0);
//================================================================================================
	if ( !level.gameEnded )
		self logXPGains();
...
//================================================================================================
// Zombotron: Added for spawn protection, last stand and blackhawk heli
	if( self.sp_active == true || self.inHelicopter == true || self.isarty == true || self.inAc130 == true || self.inAdminMenu == true )
		return;
//================================================================================================
...
//	self maps\mp\gametypes\_weapons::updateWeaponUsageStats();
	...
	self thread delayStartRagdoll( body, sHitLoc, vDir, sWeapon, eInflictor, sMeansOfDeath );
...
// Zombotron: Added
	if(level.killsprees)
	{ self thread btd\_killspree::check_for_endoframpage(eInflictor,attacker); }

	giveTeamScore( "win", "axis",  attacker, self );
	...
	{
		if( level.death_messages == 1 )
		{
			msg = randomInt(10);
			if(msg == 1)
				iPrintLnBold("^3" + self.name + " ^7flew well but landed poorly.");
			else if(msg == 2)
				iPrintLnBold("^3" + self.name + " ^7got chewed up by by a ^1Zombie!");
			else if(msg == 3)
				iPrintLnBold("^3" + self.name + " ^7makes the supreme sacrifice to the ^1Zombie ^7gods!");
			else if(msg == 4)
				iPrintLnBold("^3" + self.name + " ^7makes a note to watch out for ^1Zombies^7!");
			else if(msg == 5)
				iPrintLnBold("^3" + self.name + " ^7had a fatal encounter with a ^1Zombie^7!");
			else if(msg == 6)
				iPrintLnBold("A ^1Zombie ^7eliminated ^3" + self.name + " ^7in close combat!");
			else if(msg == 7)
				iPrintLnBold("One small step for ^3" + self.name + " ^7one giant meal for a ^2Zombie^7!");
			else if(msg == 8)
				iPrintLnBold("A ^1Zombie ^7ate right through ^3" + self.name + "'s ^7braincase!");
			else if(msg == 9)
				iPrintLnBold("^3" + self.name + " ^7was torn apart by a ^1Zombie^7!");
			else if(msg == 10)
				iPrintLnBold("^3" + self.name + " ^7stood a little too close to the action!");
        	}
	}
//================================================================================================
...
//================================================================================================
// Zombotron: Changed to reduce spam.
	if(isDefined(eInflictor))
	{
		if(isDefined(eInflictor.targetname))
		{
			if(eInflictor.targetname == "zom")
			{
				dokillcam = true;
				if(lpattacknum < 0)
				{
...
getPerks( player )
{
	...
	if ( isPlayer( player ) && (!level.oldschool && !level.traderon ))
	{
...
//================================================================================================
// Zombotron: Changed to accept a point value and not check same team
processAssist( killedplayer, points )
{
	self endon("disconnect");
	if(!isDefined(points))
		points = 0;
	...
	self thread maps\mp\gametypes\_rank::giveRankXP("assist", points);
	self incPersStat( "assists", 1 );
	self.assists = self getPersStat( "assists" );
...
//================================================================================================
// Zombotron: Have to declare this early, used for protection because zombies can be too fast and kill a last stander instantly
	self.sp_active = true;
//================================================================================================
...
{
//================================================================================================
// Zombotron: Remove zombie repellent
	self.sp_active = false;
//================================================================================================
// Zombotron:
	self.useLastStandParams = true;
...
//================================================================================================
// Zombotron: Zombies only melee. Check for melee..
...
//================================================================================================
// Zombotron: From here down added
zombieKilled( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, sHitLoc )
{
	...
	maps\mp\gametypes\_missions::waitAndProcessPlayerKilledCallback( data );
...
cleanup_entities()
{
	...
	// Remove unused entities from TDM
	// sab spawns are used as backups in the war code, so we have to keep them.
	ents = getEntArray();
	for (i = 0; i < ents.size; i++)
	{
		if( isDefined(ents[i].classname) )
		{
			//iprintln("Entity #: " + i + " Entity class: " + ents[i].classname);

			class = getsubstr(ents[i].classname, 0, 6);
			if( class == "mp_dom" || class == "mp_ctf" || class == "mp_sd_" || class == "mp_dm_" )
			{
				//iprintln("Deleted entity: " + i + " of entity class: " + ents[i].classname);
				ents[i] delete();
			}
		}
		wait 0.05;
...
g_spawn_error_avoid()
{
	...
	thread cleanup_entities();

	wait 10;

	// If we get close to the g_spawn limit, rotate maps before server crashes..
	while(1)
	{
		ents = getEntArray();

		if( ents.size >= 994 ) // Leave some headroom
		{
			iprintln("^1Avoiding potential g_spawn error server crash! Entity count:^2 ", ents.size);
			logString("Avoiding potential g_spawn error server crash!Entity count: " + ents.size);
			level thread maps\mp\gametypes\_globallogic::forceEnd();
			break;
		}
...
autoPromotePlayer(rankCap)
{
	...
	if( !self.unlocked )
	{
		if( self.pers["rank"] < rankCap )
		{
			for(;;)
			{
				if( self.pers["rank"] < 54 )
					self thread maps\mp\gametypes\_rank::giveRankXP( "kill", 500 );
				else
					self thread maps\mp\gametypes\_rank::giveRankXP( "kill", 2000 );
			...
			self iPrintln("You have been auto promoted.");
			self iPrintln("Your new rank is now: ^3" + self.pers["rank"] + " ^7enjoy!");
			self.unlocked = true;
		}
	}
}
...
cleanup()
{
	if( getDvarInt("btd_remove_hardpoint_on_death") == 1 )
	{
		self.pers["hardPointItem"] = undefined;
		self.num_arty = 0;
	}

	// Need to cleaup any links etc..
	if( isDefined( self.linker ) )
	{ self.linker delete(); }

	if( isDefined( self.lifter ) ) // Arty
	{ self.lifter delete(); }

	if( isDefined(self.arty_timer ) )
	{ self.arty_timer destroy(); }

	if( isDefined( self.tagthing ) ) // Black-Hawk
	{ self.tagthing delete(); }
/*
	if( isDefined( self.minigunturret ) ) // Deployed turret
	{ self.minigunturret delete(); }
*/
	if( isDefined(self.ac130_timer ) ) // ac130
	...
	if( isDefined(self.weap_timer ) ) // Weapon award
	{ self.weap_timer destroy(); }

	if( isDefined(self.rampage_timer ) ) // Rampage
	{ self.rampage_timer destroy(); }

	if( isDefined( self.zoomtext ) ) // Sniper zoom hud
	{
		self.zoomtext destroy();
		self.zoomnum destroy();
	}
