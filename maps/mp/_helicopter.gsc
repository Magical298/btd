...
init()
{
	...
	//precachehelicopter( "vehicle_mi-28_flying" ); // Nice but not worth it
	//precachehelicopter( "vehicle_apache" ); // Nice but not worth it
...
{
//================================================================================================
// Zombotron: No point in this, waste of cpu cycles.
	//for( ;; )
	//{
		...
		//level.heli_missile_max = heli_get_dvar_int( "scr_heli_missile_max", "9" );	// max number of missiles helicopter can carry
		...
		level.heli_visual_range = heli_get_dvar_int( "scr_heli_visual_range", "4500" );		// distance radius helicopter will acquire targets (see)
		...
		level.heli_turret_engage_dist = heli_get_dvar_int( "scr_heli_turret_engage_dist", "1500" );		// engaging distance for turret
		//level.heli_missile_engage_dist = heli_get_dvar_int( "scr_heli_missile_engage_dist", "2500" );	// engaging distance for missiles
		level.heli_missile_regen_time = heli_get_dvar( "scr_heli_missile_regen_time", "5" );			// gives one more missile to helicopter every interval - seconds
		...
		level.heli_target_recognition = heli_get_dvar( "scr_heli_target_recognition", "0.25" );			// percentage of the player's body the helicopter sees before it labels him as a target
		...
		level.heli_force_leave_time = heli_get_dvar( "scr_heli_force_leave_time", "240" );
		//wait 1;
	//}
//================================================================================================
}
...
//================================================================================================
// Zombotron: Call here again for timing reasons because I killed the infinite loop
	thread heli_update_global_dvars();

	// Mix it up a bit since we are wasting an Xmodel on Axis anyway may as well use it..
	ranTeam = randomInt(2);
	...
	//chopper = spawn_helicopter( owner, heliOrigin, heliAngles, "cobra_mp", "vehicle_apache" );
	//chopper playLoopSound( "mp_cobra_helicopter" );
	//chopper = spawn_helicopter( owner, heliOrigin, heliAngles, "cobra_mp", "vehicle_mi-28_flying" );
	//chopper playLoopSound( "mp_cobra_helicopter" );
//================================================================================================
	...
	//Zod: Put this here so we don;t have to run threads for no reason
	chopper.damageTaken = 0;
	chopper.laststate = "ok";
	chopper setdamagestage( 3 );
//================================================================================================
// Zombotron: Removed some, waste of resources
	...
	chopper thread force_heli_leave( level.heli_force_leave_time ); // In case something goes wrong, force removal after a time
//================================================================================================

	chopper thread attack_zombies();
	chopper thread heli_target_zombies();
}
...
//================================================================================================
// Zombotron: New targetting methods
...
heli_target_zombies()
{
	...
	for ( ;; )
	{
		target = setZombieTarget();
		if( isDefined(target) && canTarget_zombie( target ) )
		{
...
setZombieTarget()
{
	...
	nearestDistance = level.heli_visual_range;
	...
	if( zombies.size > 0 )
	{
		for( i = 0; i < zombies.size; i++ )
		{
			...
			tempDist = Distance(self.origin, zom.origin);
...
attack_zombies()
{
	...
	for( ;; )
	{
		if ( isdefined( self.primaryTarget ) )
		{
			...
			while( isdefined( self.turretTarget ) && self.turretTarget.health > 0 )
			{
...
heli_targeting()
{
	...
	{		
		...
		//players = level.players;
            players = getentarray("zom","targetname");
...
//================================================================================================
// Zombotron: Missile support is just for looks, so lets not waste the resources.
	...
	for( ;; )
	{
		if ( isdefined( self.secondaryTarget ) )
		{
			...
			//while( isdefined( self.missileTarget ) && isalive( self.missileTarget ) )
                  while( isdefined( self.missileTarget ) && self.missileTarget.health > 0 )
			{
	...
	for( ;; )
	{
		if ( isdefined( self.primaryTarget ) )
		{
			...
			while( isdefined( self.turretTarget ) && self.turretTarget.health > 0 )
			{
				...
				self setTurretTargetEnt( self.turretTarget, ( 0, 0, 40 ) );
//================================================================================================
// Zombotron: Removed waste of reources.
				// if wait for turret turning is too slow, enable missile assault support
...
//================================================================================================
				...
				*/
				...
				//if ( isdefined( self.turretTarget ) && isalive( self.turretTarget ) )
                        if ( isdefined( self.turretTarget ) && self.turretTarget. health > 0  )
				{
					antithreat += 100;
